...
- **Kid session** (`scripts/helpers.js` `kidSession`) — logs in with a PIN
...
```bash
node tests/performance/scripts/seed-load-tenants.mjs            # 3 tenants (default)
node tests/performance/scripts/seed-load-tenants.mjs 10         # 10 tenants
node tests/performance/scripts/seed-load-tenants.mjs 5 tests/performance/fixtures/my-run.json
```
...
`scripts/run-k6.sh` prints an install hint (`brew install k6` on macOS) if
...
- **GET-only.** These scenarios never POST/PUT/PATCH/DELETE against a
  ...
  by design (see `scripts/helpers.js`) — keep it that way. Login calls
...
tests/performance/
  ...
  scripts/
    ...
    run-k6.sh                      # wrapper: install hint + report path resolution
